                ...
                <tbody>
                    @forelse($leads as $lead)
                        <tr>
                            ...
                            <td class="text-muted small">{{ mb_substr($lead->message ?? '', 0, 60) }}</td>
                            <td class="whitespace-nowrap">{{ \Carbon\Carbon::parse($lead->created_at)->format('M d, Y h:i A') }}</td>
                        </tr>
                    @empty
                        <tr>
...
<script>
    (function() {
        ...
        async function fetchLeads(page) {
            try {
                const url = new URL('{{ url("/leads-data") }}', window.location.origin);
